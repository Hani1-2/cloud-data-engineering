...
SELECT 
    ...
    CASE 
        ...
        WHEN unit_price >= 20 AND unit_price <= 100 THEN 'Mid-range'
        ELSE 'Premium'
...
-- Use the view
SELECT * FROM retailmart.vw_OrderSummary;
...
WITH StoreRevenue AS (
    ...
    FROM [retailmart].[orders] o
    JOIN [retailmart].[stores] s ON s.store_id = o.store_id
    JOIN [retailmart].[order_items] oi ON o.order_id = oi.order_id
...
JOIN [retailmart].[order_items] oi ON o.order_id = oi.order_id
GROUP BY o.order_id, c.full_name, s.store_name, o.order_date;
GO
...
CREATE VIEW retailmart.vw_StoreRevenueTotals 
WITH SCHEMABINDING AS
SELECT 
    s.store_name,
    SUM(oi.quantity * oi.unit_price) AS total_store_revenue,
    COUNT_BIG(*) AS total_items_count
FROM retailmart.order_items oi
JOIN retailmart.orders o ON o.order_id = oi.order_id
JOIN retailmart.stores s ON s.store_id = o.store_id
GROUP BY s.store_name;
GO

-- Create unique clustered index to materialize the view
CREATE UNIQUE CLUSTERED INDEX UX_vw_StoreRevenueTotals_StoreName 
ON retailmart.vw_StoreRevenueTotals (store_name);
...
CREATE INDEX IX_Orders_PendingOrders 
...
CREATE PROCEDURE retailmart.sp_MonthlySalesReport
    @StoreID INT,
    @Year INT = 2024 -- Default year set to 2024 as requested by the test exec call
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        BEGIN
            THROW 50001, 'The requested Store ID does not exist.', 1;
        END
